...
final class GlobalAudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    // Output level (0...1) used to animate the real-time voice overlay.
    ...
    // Each processing run gets its own identifier to ignore stale callbacks.
    var currentGenerationID = UUID()

    // Real-time streaming state flags.
    private(set) var isRealtimeMode: Bool = false
    private var realtimeFinalized: Bool = false

    // Queue used to ensure only one real-time request is active at a time.
    ...
    /// Starts streaming audio playback without initial text; segments arrive later via appendRealtimeSegment.
    func startRealtimeStream() {
        ...
        // In real-time mode the player is not considered active until audio actually starts playing.
        isLoading = true
    ...
    /// Appends a playable text segment. Real-time mode enqueues it; regular mode streams immediately.
    func appendRealtimeSegment(_ text: String) {
        ...
        // Enqueue so that only one request is in flight at a time.
        enqueueRealtimeIndex(idx)
    }

    /// Marks the real-time stream as finished so the player can conclude once segments drain.
    func finishRealtimeStream() {
        guard isRealtimeMode else { return }
        realtimeFinalized = true
        // If all audio is ready and playback completed this will trigger naturally; otherwise timers and callbacks finish it.
    }
    ...
    func togglePlayback() {
        ...
        if !isAudioPlaying {
            ...
            if let chunkOpt = audioChunks[safe: currentPlayingIndex], let _ = chunkOpt {
                ...
                if isRealtimeMode {
                    // Only mark as playing after audio actually starts.
                ...
                startStallWatchdog()
                if isRealtimeMode {
                    // In real-time mode keep showing loading until data arrives.
                    isLoading = true
                    isAudioPlaying = false
                } else {
                    // Preserve the original behavior for buffered playback.
                    isAudioPlaying = true
                }
            }
        } else {
    ...
    func seek(to time: TimeInterval, shouldPlay: Bool = false) {
        ...
        if let chunkOpt = audioChunks[safe: target], let _ = chunkOpt {
            ...
            stopAudioTimer()
            startStallWatchdog()
            // Treat missing segments as buffering while in real-time mode.
            if isRealtimeMode {
                isLoading = true
    ...
    func enqueueRealtimeIndex(_ index: Int) {
        if !isRealtimeMode {
            // Immediately dispatch requests when not in real-time mode.
