...
class ChatViewModel: ObservableObject {
    ...
    // One-shot flag that enables realtime narration for the next assistant response (set by the overlay).
    private var enableRealtimeTTSNext: Bool = false
    // Indicates whether the current assistant response is streaming with realtime narration.
    private var realtimeTTSActive: Bool = false
    // Incremental segmenter that ignores <think> blocks and cuts on punctuation/length.
    ...
    init(chatSession: ChatSession) {
        ...
        // Streamed text callback; always hop back to the main actor.
        chatService.onDelta = { [weak self] piece in
            guard let self = self else { return }
            self.handleAssistantDelta(piece)

            // When realtime narration is active, feed deltas into the segmenter and stream completed chunks.
        ...
        chatService.onError = { [weak self] error in
            ...
            // If narration is active when an error occurs, finish the realtime stream gracefully.
            if self.realtimeTTSActive {
        ...
        chatService.onStreamFinished = { [weak self] in
            ...
            // When the stream finishes, flush any remaining realtime narration segments; otherwise honor auto-read.
            if self.realtimeTTSActive {
    ...
    // MARK: - Realtime narration control
    ...
    func sendMessage() {
        ...
        // Decide whether the upcoming assistant response should use realtime narration.
