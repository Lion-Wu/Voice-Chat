...
class ChatViewModel: ObservableObject {
    ...
    // Flag that enables realtime text-to-speech on the next assistant response when triggered from the voice overlay.
    private var enableRealtimeTTSNext: Bool = false
    // Indicates whether the current assistant response is streaming audio in realtime.
    private var realtimeTTSActive: Bool = false
    // Incremental text segmenter that ignores <think> blocks and emits sentences based on punctuation and length.
    ...
    init(chatSession: ChatSession) {
        ...
        // Stream delta callback (explicitly hopping back to the main actor).
        chatService.onDelta = { [weak self] piece in
            guard let self = self else { return }
            self.handleAssistantDelta(piece)

            // When streaming audio, feed new body text into the segmenter and forward finished segments to the audio manager.
        ...
        chatService.onError = { [weak self] error in
            ...
            // Finish the realtime stream if an error occurs while streaming audio.
            if self.realtimeTTSActive {
        ...
        chatService.onStreamFinished = { [weak self] in
            ...
            // When the stream finishes, flush any remaining realtime segments or trigger the legacy auto-read flow.
            if self.realtimeTTSActive {
    ...
    // MARK: - Realtime streaming control
    ...
    func sendMessage() {
        ...
        // Determine whether to start realtime streaming for this assistant response.
