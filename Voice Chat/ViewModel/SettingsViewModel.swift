...
class SettingsViewModel: ObservableObject {
    ...
    // Legacy preset fields kept for backward compatibility with older storage
    ...
    @Published var enableStreaming: Bool {
        didSet {
            saveVoiceSettings()
            if enableStreaming {
                // Force the splitter to cut0 when streaming is enabled
                autoSplit = "cut0"
            }
        }
    }
    ...
    @Published var selectedPresetID: UUID? {
        didSet {
            if !suppressPresetDidSet {
                ...
                // Refresh preset fields whenever the selection changes
    ...
    // Prevent recursive didSet loops when switching presets
    ...
    func saveServerSettings() {
        // Keep legacy fields in sync for compatibility; TTS no longer consumes them directly
    ...
    private func savePresetFields() {
        ...
        // The name may change, so refresh the displayed list
        reloadPresetListAndSelection()
    }
