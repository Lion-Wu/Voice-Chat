...
final class ChatSessionsViewModel: ObservableObject {
    ...
    // MARK: - Save throttling (prevents write storms during streaming)
    ...
    func attach(context: ModelContext) {
        // Avoid attaching multiple contexts
        if self.context == nil {
    ...
    func addSession(_ session: ChatSession) {
        guard let context else { return }
        // Insert the session into the context when needed
    ...
    func deleteSession(at offsets: IndexSet) {
        ...
        for index in offsets {
            ...
            context.delete(s) // Cascades to delete associated messages
        }
