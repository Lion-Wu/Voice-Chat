...
final class ChatSessionsViewModel: ObservableObject {
    ...
    // MARK: - Save throttling to avoid excessive writes
    ...
    func attach(context: ModelContext) {
        // Avoid appending duplicates when the session already exists.
        if self.context == nil {
    ...
    func addSession(_ session: ChatSession) {
        guard let context else { return }
        // Insert into the context if it is not already present.
