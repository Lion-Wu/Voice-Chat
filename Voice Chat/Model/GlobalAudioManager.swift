...
final class GlobalAudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    // Discard previous callbacks when starting to avoid stale state from cancelled requests.
    ...
    // MARK: - Entry points for non-streaming audio
    ...
    /// Starts a streaming playback session. Text is appended incrementally.
    ...
    /// Appends a playable text segment, queueing it when in streaming mode.
    ...
    /// Finishes the current streaming session and performs cleanup.
    func finishRealtimeStream() {
        guard isRealtimeMode else { return }
        realtimeFinalized = true
        // If all audio is loaded and finished, `finishPlayback` will be triggered naturally; otherwise wait for timers/callbacks.
    }
    ...
    func togglePlayback() {
        ...
        if !isAudioPlaying {
            ...
            if let chunkOpt = audioChunks[safe: currentPlayingIndex], let _ = chunkOpt {
                ...
                if isRealtimeMode {
                    // Mark as playing only after audio actually starts.
                ...
                startStallWatchdog()
                if isRealtimeMode {
                    // Streaming mode keeps showing loading when data has not arrived.
                    isLoading = true
                    isAudioPlaying = false
                } else {
                    // Non-streaming mode retains the original behavior.
                    isAudioPlaying = true
                }
            }
        } else {
    ...
    func enqueueRealtimeIndex(_ index: Int) {
        if !isRealtimeMode {
            // Send immediately in non-streaming mode.
