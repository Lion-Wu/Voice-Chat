...
final class GlobalAudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    // Output level (0-1) used by the realtime voice overlay for circle scaling
    ...
    // Drop callbacks from previous sessions when starting a new processing pipeline.
    var currentGenerationID = UUID()

    // Realtime streaming mode toggle and completion flag.
    private(set) var isRealtimeMode: Bool = false
    private var realtimeFinalized: Bool = false

    // Realtime mode enforces strict serialization (only one request active at a time).
    ...
    /// Starts a realtime voice stream. Segments are appended incrementally afterwards.
    func startRealtimeStream() {
        ...
        // Do not mark playback as active until audio actually starts.
        isLoading = true
    ...
    /// Appends a playable text segment (queued strictly in realtime mode).
    func appendRealtimeSegment(_ text: String) {
        ...
        // Queue segments so only one request is in flight at a time.
        enqueueRealtimeIndex(idx)
    }

    /// Finishes the current realtime stream, flushing any tail segments.
    func finishRealtimeStream() {
        guard isRealtimeMode else { return }
        realtimeFinalized = true
        // If all audio has been processed the normal finish flow will trigger; otherwise wait for timers/callbacks.
    }
    ...
    func togglePlayback() {
        ...
        if !isAudioPlaying {
            ...
            if let chunkOpt = audioChunks[safe: currentPlayingIndex], let _ = chunkOpt {
                ...
                if isRealtimeMode {
                    // Consider playback active only once audio actually starts.
                ...
                startStallWatchdog()
                if isRealtimeMode {
                    // Realtime mode: remain in loading state until data arrives.
                    isLoading = true
                    isAudioPlaying = false
                } else {
                    // Standard mode retains legacy behavior.
                    isAudioPlaying = true
                }
            }
        } else {
    ...
    func seek(to time: TimeInterval, shouldPlay: Bool = false) {
        ...
        if let chunkOpt = audioChunks[safe: target], let _ = chunkOpt {
            ...
            stopAudioTimer()
            startStallWatchdog()
            // In realtime mode, missing segments count as loading and not active playback.
            if isRealtimeMode {
                isLoading = true
    ...
    func enqueueRealtimeIndex(_ index: Int) {
        if !isRealtimeMode {
            // Standard mode dispatches immediately.
