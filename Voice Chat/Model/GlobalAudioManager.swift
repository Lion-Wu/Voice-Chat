...
final class GlobalAudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    // Output level (0...1) for scaling the speaking indicator in realtime overlays.
    ...
    // Generated for each startProcessing call; discard stale callbacks from earlier sessions.
    var currentGenerationID = UUID()

    // Realtime streaming mode flag and a marker indicating finalization.
    private(set) var isRealtimeMode: Bool = false
    private var realtimeFinalized: Bool = false

    // Strict queue for realtime mode: process one request at a time.
    var pendingRealtimeIndexes: [Int] = []

    // MARK: - Entry point (full-text mode)
    ...
    /// Begin a realtime speech stream; text arrives later via `appendRealtimeSegment`.
    func startRealtimeStream() {
        ...
        // Important: realtime mode reports as loading until audio truly starts.
        isLoading = true
    ...
    /// Append a segment for narration (strict queue in realtime mode; immediate in batch mode).
    func appendRealtimeSegment(_ text: String) {
        ...
        // Ensure only one realtime request is dispatched at a time.
        enqueueRealtimeIndex(idx)
    }

    /// Finish the realtime stream so pending timers/callbacks can close playback.
    func finishRealtimeStream() {
        guard isRealtimeMode else { return }
        realtimeFinalized = true
        // When everything has loaded and played, `finishPlayback()` will fire automatically; otherwise timers handle it.
    }
    ...
    func togglePlayback() {
        ...
        if !isAudioPlaying {
            ...
            if let chunkOpt = audioChunks[safe: currentPlayingIndex], let _ = chunkOpt {
                ...
                if isRealtimeMode {
                    // Consider the player active only after audio actually starts.
                ...
                startStallWatchdog()
                if isRealtimeMode {
                    // Realtime mode keeps showing the loading state while waiting for audio data.
                    isLoading = true
                    isAudioPlaying = false
                } else {
                    // Batch mode maintains the previous behavior (stay marked as playing).
                    isAudioPlaying = true
                }
            }
        } else {
    ...
    func seek(to time: TimeInterval, shouldPlay: Bool = false) {
        ...
        if let chunkOpt = audioChunks[safe: target], let _ = chunkOpt {
            ...
            stopAudioTimer()
            startStallWatchdog()
            // Treat missing segments as loading in realtime mode so the UI stays paused.
            if isRealtimeMode {
                isLoading = true
    ...
    func enqueueRealtimeIndex(_ index: Int) {
        if !isRealtimeMode {
            // In batch mode dispatch immediately.
