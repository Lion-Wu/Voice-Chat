//
//  IncrementalTextSegmenter.swift
//  Voice Chat
//
//  Created by Lion Wu on 2025/9/21.
//

import Foundation

/// Reassembles streamed text into coherent, speakable segments.
/// - Ignores content inside `<think>` tags and only emits the visible body.
/// - Splits on terminal punctuation or newline characters.
/// - Falls back to length-based segmentation when punctuation is not available.
struct IncrementalTextSegmenter {

    private var buffer: String = ""
    private var inThink: Bool = false

    // Length thresholds derived from empirical testing
    private let maxCJKChars: Int = 80
    private let maxENWords: Int = 30

    // Punctuation that terminates a sentence
    private let terminalSet: Set<Character> = Set("。！？!?……;；.。")
    // Treat newline as a soft breakpoint
    private let newline: Character = "\n"

    mutating func reset() {
        buffer = ""
        inThink = false
    }

    /// Appends a delta and returns any segments that are immediately ready for playback
    mutating func append(_ delta: String) -> [String] {
        guard !delta.isEmpty else { return [] }

        var produced: [String] = []
        var i = delta.startIndex

        while i < delta.endIndex {
            // Track entry and exit for <think> tags
            if delta[i...].hasPrefix("<think>") {
                inThink = true
                i = delta.index(i, offsetBy: 7)
                continue
            }
            if delta[i...].hasPrefix("</think>") {
                inThink = false
                i = delta.index(i, offsetBy: 8)
                continue
            }

            // Only buffer visible content
            let ch = delta[i]
            if !inThink {
                buffer.append(ch)

                // Split immediately when encountering newline or punctuation
                if ch == newline || terminalSet.contains(ch) {
                    let seg = buffer.trimmingCharacters(in: .whitespacesAndNewlines)
                    if !seg.isEmpty {
                        produced.append(seg)
                    }
                    buffer = ""
                } else {
                    // Enforce a split when no punctuation appears for an extended stretch
                    if shouldForceSplit(buffer) {
                        let seg = buffer.trimmingCharacters(in: .whitespacesAndNewlines)
                        if !seg.isEmpty {
                            produced.append(seg)
                        }
                        buffer = ""
                    }
                }
            }

            i = delta.index(after: i)
        }

        return produced
    }

    /// Flush any remaining text at the end of the stream
    mutating func finalize() -> [String] {
        guard !buffer.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return [] }
        let tail = buffer.trimmingCharacters(in: .whitespacesAndNewlines)
        buffer = ""
        return [tail]
    }

    // MARK: - Helpers

    /// Simple heuristic to decide when to force a split based on word or character count
    private func shouldForceSplit(_ text: String) -> Bool {
        // Determine whether the text contains CJK characters
        let hasCJK = text.unicodeScalars.contains { $0.properties.isIdeographic }
        if hasCJK {
            return text.count >= maxCJKChars
        } else {
            let words = text.split { $0.isWhitespace || $0.isNewline }
            return words.count >= maxENWords
        }
    }
}
