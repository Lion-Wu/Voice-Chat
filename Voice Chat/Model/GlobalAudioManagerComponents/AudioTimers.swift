...
extension GlobalAudioManager {
    ...
    func startAudioTimer() {
        ...
        audioTimer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] _ in
            Task { @MainActor in
                ...
                // Enable metering and keep it refreshed for the overlay level indicator.
                if !p.isMeteringEnabled { p.isMeteringEnabled = true }
                p.updateMeters()
                let power = p.averagePower(forChannel: 0) // dB [-160, 0]
                // Convert the dB value into a linear 0...1 scale.
    ...
    func stopAudioTimer() {
        ...
        audioTimer = nil
        // Reset the output level when the timer stops.
        outputLevel = 0
    }
