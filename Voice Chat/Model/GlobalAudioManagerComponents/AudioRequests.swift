...
extension GlobalAudioManager {
    ...
    // MARK: - Request queue (only used in full-text mode)
    func sendNextSegment() {
        guard currentChunkIndex < textSegments.count else { return }
        guard !isRealtimeMode else { return } // Realtime mode does not use the recursive chain
    ...
    func sendTTSRequest(for segmentText: String, index: Int) {
        ...
        guard let body = try? JSONSerialization.data(withJSONObject: params) else {
            ...
            // Realtime mode: continue advancing the queue to avoid deadlocks
            ...
            DispatchQueue.main.async {
                ...
                defer {
                    ...
                    if self.isRealtimeMode {
                        ...
                        // Standard mode streams segments sequentially
                ...
                // Protect against index issues while the realtime array grows dynamically
