...
struct ServerSettings: Codable {
    ...
    var textLang: String
    // These fields are kept for legacy compatibility; presets now manage them.
...
struct VoiceSettings: Codable {
    ...
    /// Whether to automatically read aloud after a response is generated.
...
final class VoicePreset {
    ...
    // Fields previously stored in `ServerSettings` plus two new weight paths.
    var refAudioPath: String
    var promptText: String
    var promptLang: String
...
final class AppSettings {
    ...
    var serverAddress: String
    var textLang: String
    // Compatibility fields that are now managed via `VoicePreset`.
    var refAudioPath: String
    var promptText: String
    var promptLang: String
...
final class SettingsManager: ObservableObject {
    ...
    private init() {
        // Bootstrap with defaults until attach(context:) loads persisted data.
    ...
    // SwiftData context injected from the app or content view
    func attach(context: ModelContext) {
        ...
        // Synchronize the stored selectedPresetID back into memory
    ...
    private func ensureDefaultPresetIfNeeded() {
        ...
        if presets.isEmpty {
            ...
            // Select the first preset when none is selected
    ...
    // MARK: - Update legacy settings (for compatibility)

    func updateServerSettings(serverAddress: String, textLang: String, refAudioPath: String, promptText: String, promptLang: String) {
        // Keep ref/prompt fields in sync for compatibility even though presets own them now.
    ...
    func deletePreset(_ id: UUID) {
        ...
        if let target = presets.first(where: { $0.id == id }) {
            // Switch to another preset if the selected one is deleted
    ...
    // MARK: - Apply preset (sequentially call the weight APIs)
    ...
    func applySelectedPreset() async {
        ...
        // Update legacy fields first so any remaining readers stay consistent.
        ...
        // Build URLs while handling optional trailing slashes on serverAddress
        ...
        if let url2 = buildURL("/set_sovits_weights", weightsPath: preset.sovitsWeightsPath) {
            do {
                ...
                if let s = String(data: data, encoding: .utf8),
                   s.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() != "success" {
                    // Same as above—empty body is still considered success.
                }
            } catch {
