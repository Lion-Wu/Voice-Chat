...
// MARK: - Value types used by the UI layer
...
struct VoiceSettings: Codable {
    ...
    /// Indicates whether audio playback should start automatically once generation completes
...
final class VoicePreset {
    ...
    // Fields that were previously distributed across server settings and new weight paths
    var refAudioPath: String
    var promptText: String
    var promptLang: String
...
final class AppSettings {
    ...
    var serverAddress: String
    var textLang: String
    // Legacy fields kept for compatibility; new versions rely on VoicePreset instead
    var refAudioPath: String
    var promptText: String
    var promptLang: String
    ...
    // Identifier of the currently selected preset
...
final class SettingsManager: ObservableObject {
    ...
    // Legacy structures still used for server address and language bindings
    ...
    // Preset collection and selection state
    ...
    // State applied while pushing a preset to the server
    ...
    // Ensures preset application on launch only happens once
    ...
    // SwiftData context injected from the application layer
    func attach(context: ModelContext) {
        ...
        // Mirror the persisted preset selection into memory
    ...
    private func ensureDefaultPresetIfNeeded() {
        ...
        if presets.isEmpty {
            // Build a default preset from legacy fields when necessary
            let def = VoicePreset(
                name: L10n.Settings.presetDefaultName,
                refAudioPath: e.refAudioPath,
                promptText: e.promptText,
                promptLang: e.promptLang,
                gptWeightsPath: "",
            ...
            // Fallback to the first preset if nothing is selected yet
    ...
    func createPreset(name: String = L10n.Settings.newPresetDefaultName) -> VoicePreset? {
    ...
    func deletePreset(_ id: UUID) {
        ...
        if let target = presets.first(where: { $0.id == id }) {
            // If the active preset is removed, fall back to another entry
    ...
    // MARK: - Apply selected preset (sequentially calling both weight endpoints)
    ...
    func applySelectedPreset() async {
        ...
        // Update legacy fields first so any remaining consumers stay in sync
        ...
        // Build URLs while tolerating trailing slashes on the server address
        ...
        if let url1 = buildURL("/set_gpt_weights", weightsPath: preset.gptWeightsPath) {
            do {
                ...
                guard let http = resp as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
                    ...
                    lastApplyError = L10n.Settings.errorWeightsHTTP(L10n.Settings.gptWeightsName, code)
                    return
                }
                if let s = String(data: data, encoding: .utf8),
                   s.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() != "success" {
                    // Some servers return an empty body, so do not fail solely on content
                }
            } catch {
                lastApplyError = L10n.Settings.errorWeightsGeneric(L10n.Settings.gptWeightsName, error.localizedDescription)
                return
            }
        } else {
            lastApplyError = L10n.Settings.errorWeightsInvalidURL(L10n.Settings.gptWeightsName)
            return
        }
        ...
        if let url2 = buildURL("/set_sovits_weights", weightsPath: preset.sovitsWeightsPath) {
            do {
                ...
                guard let http = resp as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
                    let code = (resp as? HTTPURLResponse)?.statusCode ?? -1
                    lastApplyError = L10n.Settings.errorWeightsHTTP(L10n.Settings.sovitsWeightsName, code)
                    return
                }
                if let s = String(data: data, encoding: .utf8),
                   s.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() != "success" {
                    // Same as above—avoid treating non-success payloads as fatal
                }
            } catch {
                lastApplyError = L10n.Settings.errorWeightsGeneric(L10n.Settings.sovitsWeightsName, error.localizedDescription)
                return
            }
        } else {
            lastApplyError = L10n.Settings.errorWeightsInvalidURL(L10n.Settings.sovitsWeightsName)
            return
        }
    }
}
