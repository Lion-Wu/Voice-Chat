...
/// Compatibility for LM Studio v0.3.23+ reasoning stream fields.
...
final class ChatService: NSObject, URLSessionDataDelegate, @unchecked Sendable {
    ...
    /// Callbacks are explicitly constrained to the main actor.
    var onDelta: (@MainActor (String) -> Void)?
    var onError: (@MainActor (Error) -> Void)?
    var onStreamFinished: (@MainActor () -> Void)?

    // Track reasoning/body stream state.
    ...
    // Watchdog configuration for the one-hour timeout requirement.
    private let firstTokenTimeout: TimeInterval = 3600        // wait up to 1h for first token
    private let silentGapTimeout: TimeInterval  = 3600        // allow 1h between deltas
    private var streamStartAt: Date?
    private var lastDeltaAt: Date?
    private var watchdog: Timer?

    // Cancellation flag used to drop residual deltas after stopping.
    ...
    override init() {
        ...
        configuration.timeoutIntervalForRequest  = 3900   // ~1h + 5m headroom
    ...
    /// Invoke on the main actor so SwiftData models do not cross isolation boundaries.
    @MainActor
    ...
    /// Synchronous setup: configure and launch the URLSession stream.
    private func startStreaming(apiURLString: String, model: String, messages: [ChatMessage]) {
        ...
        request.timeoutInterval = 3900 // per-request timeout (~1h + 5m)
    ...
    // MARK: - Watchdog (surface an error after extreme waits)
        ...
        watchdog = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { [weak self] _ in
            ...
            if let start = self.streamStartAt, self.lastDeltaAt == nil {
                if now.timeIntervalSince(start) > self.firstTokenTimeout {
                    ...
                    self.dataTask = nil
                    self.stopWatchdog()
                    Task { @MainActor in self.onError?(ChatNetworkError.timeout(L10n.Common.connectionTimeoutText)) }
                }
                return
            }
