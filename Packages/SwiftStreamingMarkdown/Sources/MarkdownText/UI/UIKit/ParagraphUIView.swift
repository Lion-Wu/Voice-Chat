...
class ParagraphUIView: UITextView {
  ...
  private func updateTextViewWithCurrentAnimations() {
    ...
    for animation in activeAnimations {
      ...
      let defaultColor = MarkdownRenderConfig.defaultPrimaryTextColor
...
extension ParagraphUIView: UITextViewDelegate {
  ...
  func textView(_ textView: UITextView, primaryActionFor textItem: UITextItem, defaultAction: UIAction) -> UIAction? {
    if let url = textItem.value(forKey: "link") as? URL {
      return UIAction { [weak self] _ in
        self?.onUrlTap(url)
      }
    }

    if let citationAttachment = textItem.value(forKey: "textAttachment") as? InlineCitationAttachment,
