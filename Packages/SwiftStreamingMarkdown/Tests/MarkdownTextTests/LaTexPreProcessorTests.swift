...
final class LaTexPreProcessorTests: XCTestCase {
  ...
  func testInlineDollarMath() {
    let input = "Euler wrote $e^{i\\pi} + 1 = 0$."
    let expectation = "Euler wrote `\\(e^{i\\pi} + 1 = 0\\)`."

    XCTAssertEqual(preprocessor.process(input: input), expectation)
  }

  func testInlineDollarMathAllowsSymmetricPadding() {
    let input = "The result is $ x + y $."
    let expectation = "The result is `\\(x + y\\)`."

    XCTAssertEqual(preprocessor.process(input: input), expectation)
  }

  func testInlineDollarMathLeavesLiteralDollarsAndCodeUntouched() {
    let input = #"Price: $5. Code: `$value`. Escaped: \$name. Block: $$x^2$$"#

    let expectation = """
    Price: $5. Code: `$value`. Escaped: \\$name. Block:
    ```blockmath
    x^2
    ```
    """
  ...
  func testLatexInsideCodeSpansAndFencesRemainsLiteral() {
    let input = #"""
    Inline code: `\(not math\)` and `$stillNotMath$`.
  ...
  func testEmbeddedDisplayDelimitersBecomeMathBlocks() {
    let input = #"Before $$x^2 + y^2$$ after \[z = 1\] done."#
    let expectation = """
    Before
    ```blockmath
    x^2 + y^2
  ...
  func testIncompleteStreamedDelimitersRemainUntouched() {
    let inputs = [
      "Inline $x + y",
      "Block $$x + y",
      #"Bracket \[x + y"#,
  ...
  func testBoxedLatexIsPreservedForRaTeX() {
    let input = #"\[\boxed{E = mc^2}\]"#

    XCTAssertTrue(preprocessor.process(input: input).contains(#"\boxed{E = mc^2}"#))
  }
  ...
  @MainActor
  func testInlineLatexUsesDedicatedAttachmentContentType() async throws {
    let document = await MarkdownParserImpl().parse(
      text: "$x + y$",
      config: .default
    )
    let attributedString = try XCTUnwrap(document.attributedStrings.first)
    ...
    XCTAssertEqual(
      attachment.fileType,
      UTType.markdownLatex.identifier
    )
  }

  func testLatexAttachmentColorsResolveForBothAppearances() {
    let data = LatexAttachmentData(
      latex: "x",
      fontSize: 17,
      lightTextColor: "#112233",
      darkTextColor: "#DDEEFF"
    )

    #if canImport(UIKit)
    let light = data.resolvedTextColor.resolvedColor(
      with: UITraitCollection(userInterfaceStyle: .light)
    )
    let dark = data.resolvedTextColor.resolvedColor(
      with: UITraitCollection(userInterfaceStyle: .dark)
    )
    #elseif canImport(AppKit)
    let light = data.resolvedTextColor.resolvedForAppearance(.aqua)
    let dark = data.resolvedTextColor.resolvedForAppearance(.darkAqua)
    #endif

    XCTAssertEqual(light.toHexString(), "#112233")
    XCTAssertEqual(dark.toHexString(), "#DDEEFF")
  }
